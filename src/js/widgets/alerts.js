...
var Alerts = Class.create({
    init: function (el, opts) {
        ...
        $this.desktopNotificationPermission();
		...
		$this.history = {};
		$this.bc = new BroadcastChannel('alerts_channel');
		$this.bc.onmessage = function (ev) { $this.showNotification(ev.data.id,ev.data.message,ev.data.title); }
		$this.notify();
    },
	...
	notify: function(){
		...
		$.post(this.options.script,{action: 'getNotifications'}, function(json){
			setTimeout(function(){ $this.notify(); },30*1000);
									
			if(json.items){
				$.each(json.items,function(n,el){					
					$this.showNotification(el.id,el.description,el.title);	
					$this.desktopNotification(el.description,el.title);					
				});
				$this.updateCount();
				$this.updateList();  	
			}
	...
	desktopNotificationPermission: function(){
		var myNotification = new Notify('Init');
		if (Notify.needsPermission) {
            Notify.requestPermission();
        }
	},
	desktopNotification: function(msg,title) {
		if (!Notify.needsPermission) {            		
			var myNotification = new Notify(title, {
				...
				notifyClick: function(){ window.location = '/~nrzent5/bot/site/orders2'; },
	...
	showNotification: function(id,msg,title){
		var $this = this;
		console.log("HERE: "+id);
		
		if(typeof id == 'undefined' || $this.history[id] == 1)return false;
		$this.history[id] = 1;
		...
		*/
        ...
        $this.bc.postMessage({'id':id,'title':title,'message':msg});
	},
