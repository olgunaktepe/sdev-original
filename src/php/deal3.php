...
function getListingUpdates($id){
	...
	list($source,$rId) = mysql_fetch_array(mysql_query("SELECT source,remote_id FROM listings WHERE id='{$id}'"));	

	$q = mysql_query("SELECT id,timestamp,diff_count FROM listings_updates WHERE listing_id='{$rId}' ORDER BY id ASC");
...
function loadListingUpdates(){
	...
	list($source,$rId) = mysql_fetch_array(mysql_query("SELECT source,remote_id FROM listings WHERE id='{$_REQUEST['id']}'"));	
	
	
	$wheresql[] = "listing_id='{$rId}'";
...
function formatListingUpdate($r){
...
function loadTimeline(){
	...
	//Dups
	$dups = $dl->getDuplicateListingsV2($id,'',true);
	if($dups){		
		if($dups['items']){
		foreach($dups['items'] as $d){							
			$df = $d->listing;
			$score = number_format($d->score,2);
			if($df->url){								
			...
			$desc .= "<br>--------------<br>Overall Score: {$score}<br>Decision Reason: {$d->reason}";
