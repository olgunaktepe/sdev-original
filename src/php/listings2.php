...
function calcLocationsStats(){    
    ...
    do{
        ...
        t("Launching...",1);    
        $cmd = "php /home/sdev/public_html/php/util/statsThread.util.php '".json_encode($params)."'";
        t($cmd);
        exec($cmd,$res);    
        t($res);
...
function getLocations(){
    ...
    $items = loadLocations($filter,$limit,$offset);     
    $formatted = standerizeListing($items);                
...
function loadLocationSql($filter,$limit=0,$offset=0,$orderby=''){
    ...
    $sql = '
        SELECT SQL_CALC_FOUND_ROWS m.* FROM listings AS m                
