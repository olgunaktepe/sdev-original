...
const scss = function () {
    ...
    src([paths.baseSrcAssets + "scss/config/material/*.scss", "!" + paths.baseSrcAssets + "scss/icons.scss", "!" ])
    ...
    // generate rtl
    return src([paths.baseSrcAssets + "scss/config/material/*.scss", "!" ])
        .pipe(sourcemaps.init())
        .pipe(sass.sync().on('error', sass.logError)) // scss to css
        .pipe(
            autoprefixer({
...
exports.default = series(
    vendor,
    parallel(javascript, scss, icons),
    parallel(watchFiles)
);
