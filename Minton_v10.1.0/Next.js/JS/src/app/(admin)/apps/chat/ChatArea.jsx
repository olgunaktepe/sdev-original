  ...
  return <li className={classnames("clearfix", {
    odd: message.from.id === toUser.id
            ...
            <div className="chat-avatar">
                <Image src={message.from.avatar} className="rounded" alt="user avatar" height={42} width={42} />
                <i>{message.sendOn}</i>
            </div>

            <div className="conversation-text">
                {message.message.type === "text" && <div className="ctext-wrap">
                        <i>{message.from.name}</i>
                        {message.message.type === "text" && <p>{message.message.value}</p>}
                    </div>}

                {message.message.type === "file" && <Card className="mt-2 mb-1 shadow-none border text-start">
                        <div className="p-2">
            ...
            <Dropdown className="conversation-actions" align={message.from.id === toUser.id ? "start" : "end"}>
  ...
  return <Card>
            <Card.Body className="py-2 px-3 border-bottom border-light">
                <div className="d-flex py-1">
                    <Image src={selectedUser.avatar} className="me-2 rounded-circle" height="36" alt="Brandon Smith" />
                    <div className="flex-1">
                        <h5 className="mt-0 mb-0 font-15">
                            <Link href="/apps/contacts/profile" className="text-reset">
                                {selectedUser.name}
                            </Link>
                        </h5>
                    ...
                    <div id="tooltips-container">
                        <OverlayTrigger placement="top" overlay={<Tooltip id="">Voice Call</Tooltip>}>
                            <Link href="" className="text-reset font-19 py-1 px-2 d-inline-block">
                        ...
                        <OverlayTrigger placement="top" overlay={<Tooltip id="">Delete Chat</Tooltip>}>
              <span>
                <Link href="" className="text-reset font-19 py-1 px-2 d-inline-block">
                            ...
                            <form noValidate name="chat-form" id="chat-form" onSubmit={handleSubmit(sendChatMessage)}>
                                <Row>
                                    <Col className="mb-2 mb-sm-0">
                                        <FormInput type="text" name="newMessage" className="border-0" placeholder="Enter your text" register={register} key="newMessage" errors={errors} control={control} />
                                    </Col>
