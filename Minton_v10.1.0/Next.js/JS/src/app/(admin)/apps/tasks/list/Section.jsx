...
const Task = ({
  ...
  return <>
      <tr>
        ...
        <td>
          <div>
            <Image src={task.assignee_avatar} id={`task-avatar-${task.id}`} className="avatar-sm img-thumbnail rounded-circle" alt="" height={36} width={36} />
          </div>
        </td>
        <td>{task.due_date}</td>
        <td>
        ...
        <td>
          <ul className="list-inline table-action m-0">
            <li className="list-inline-item">
              <Link href="" className="action-icon px-1">
...
const TaskSection = ({
  ...
  return <>
      <h5 className="position-relative mb-0">
        <Link className="text-dark d-block" href="" onClick={toggleTask}>
          {title} Tasks{" "}
