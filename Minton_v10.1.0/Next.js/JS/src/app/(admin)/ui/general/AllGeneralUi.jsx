...
import { Pagination, Badge, Breadcrumb, Button } from "react-bootstrap";
...
const PillBadges = () => {
  return <div className="card">
      <div className="card-body">
        ...
        return <Badge pill className={classNames("me-1", "bg-" + color.color, color.color === "light" ? "text-dark" : null)} key={index}>
              {color.name}
            </Badge>;
      })}
        ...
        {(colors || []).map((color, index) => {
        return <Badge pill bg="" className={classNames("me-1", "badge-soft-" + color.color)} key={index}>
              {color.name}
            </Badge>;
      })}
        ...
        {(colors || []).map((color, index) => {
        return <Badge bg="" pill className={classNames("me-1", "badge-outline-" + color.color)} key={index}>
              {color.name}
            </Badge>;
      })}
      </div>
...
const ContexualBadges = () => {
  return <div className="card">
      <div className="card-body">
        ...
        {(colors || []).map((color, index) => {
        return <Badge className={classNames("me-1", "badge bg-" + color.color, color.color === "light" ? "text-dark" : null)} key={index}>
              {color.name}
            </Badge>;
      })}
        ...
        {(colors || []).map((color, index) => {
        return <Badge className={classNames("me-1", "badge-soft-" + color.color)} bg="" key={index}>
              {color.name}
            </Badge>;
      })}
        ...
        {(colors || []).map((color, index) => {
        return <Badge bg="" className={classNames("me-1", "badge-outline-" + color.color)} key={index}>
              {color.name}
            </Badge>;
      })}
      </div>
        ...
        <div className="card-body">
          ...
          <div className="row">
            ...
            <div className="col-6">
              <Button className="position-relative">
                Profile
                <Badge bg="danger" className="position-absolute top-0 start-100 translate-middle p-1 border border-light rounded-circle">
...
const DefaultPagination = () => {
  ...
  for (let number = 1; number <= 5; number++) {
    items.push(<Pagination.Item key={number} active={number === 3}>
        {number}
      </Pagination.Item>);
  }
  return <>
...
const BreadcrumbExample = () => {
  return <>
      ...
      <p className="sub-header">
        ...
        <Link target="_blank" rel="noreferrer" href="https://getbootstrap.com/docs/4.0/components/breadcrumb/">
          Bootstrap
        </Link>{" "}
