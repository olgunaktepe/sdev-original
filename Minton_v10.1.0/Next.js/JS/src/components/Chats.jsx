...
const Chats = ({
  ...
  return <>
      ...
      <div className="p-2">
        <Link href='' className="text-reset notification-item ps-3 mb-2 d-block">
        ...
        <Link href='' className="text-reset notification-item ps-3 d-block">
      ...
      <div className="p-2">
        {(chats.filter(e => e.group === "favourites") || []).map((item, index) => {
        return <Link href='' key={index} className="text-reset notification-item">
              <div className="d-flex align-items-start noti-user-item">
                <div className="position-relative me-2">
                  <span className={classNames("user-status", item.userStatus)}></span>
                  <Image src={item.avatar} className="rounded-circle avatar-sm" alt="chat avatar" />
                </div>
      ...
      <div className="p-2 pb-4">
        {(chats.filter(e => e.group === "other") || []).map((item, index) => {
        return <Link href='' key={index} className="text-reset notification-item">
                <div className="d-flex align-items-start noti-user-item">
                  <div className="position-relative me-2">
                    <span className={classNames("user-status", item.userStatus)}></span>
                    <Image src={item.avatar} className="rounded-circle avatar-sm" alt="user avatar" />
                  </div>
                  <div className="overflow-hidden">
                    <h6 className="mt-0 mb-1 font-14">{item.userName}</h6>
                    <div className="font-13 text-muted">
        ...
        <div className="text-center mt-3">
          <Link href='' className="btn btn-sm btn-white">
