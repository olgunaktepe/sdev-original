...
const TaskItem = ({id, task}: TaskItemProps) => {
    const {
        attributes,
        listeners,
        setNodeRef,
    ...
    return (
        <li ref={setNodeRef} style={style} {...attributes} {...listeners} className={`task-${task.priority}`}>
                ...
                <div className="col-auto">
                    <div>
                        {(task.userAvatar || []).map((avatar, index) => {
                            return (
                                <Link key={index} href="" className="me-1">
                                    <Image
                                        src={avatar}
                                        alt="avatar"
...
const TaskList = ({list, title}: {
    list: TaskTypes[], title?: string
}) => {

    return (
        <SortableContext items={list.map(i => i.id.toString())} strategy={verticalListSortingStrategy}>
                ...
                <div className="card">
                    <div className="card-body">
                        ...
                        <ul className="sortable-list tasklist list-unstyled">
                            {list.map((item) => (
                                <TaskItem task={item} id={item.id.toString()} key={item.id}/>
                            ))}
                        </ul>
...
const KanbanBoard = () => {
    ...
    const [upcomingTasks, setUpcomingTasks] = useState<TaskTypes[]>(tasks.filter((t) => t.status === "upcoming"))
    const [inProgressTasks, setInProgressTasks] = useState<TaskTypes[]>(tasks.filter((t) => t.status === "in-progress"))
    const [completedTasks, setCompletedTasks] = useState<TaskTypes[]>(tasks.filter((t) => t.status === "completed"))
