      ...
      <Row>
        <Col md={12}>
          ...
          return <div key={index} className="search-item">
                {searchItem.title && <h4 className="mb-1">
                    <Link to="">{searchItem.title}</Link>
                  </h4>}
                {searchItem.url && <div className="font-13 text-success mb-2 text-truncate">
                    {searchItem.url}
                  </div>}
                <div dangerouslySetInnerHTML={createMarkup(searchItem.content)}></div>
              </div>;
        })}
