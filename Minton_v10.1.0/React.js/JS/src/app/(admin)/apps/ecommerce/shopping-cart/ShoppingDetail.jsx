...
const ShoppingDetail = () => {
  ...
  */
  ...
  const _adjustCart = localItems => {
    ...
    const newNetTotal = newGrossTotal - summary.discount + summary.shipping_charge + summary.tax;
    setItems(localItems);
    setSummary({
      ...summary,
  ...
  return <Row>
    <Col lg={8}>
      <Table responsive borderless className="table-centered mb-0">
        ...
        <tbody>
          {(items || []).map((item, idx) => {
            return <tr key={idx}>
                <td>
                  <img src={item.image} alt={item.name} title="product-img" className="avatar-lg" height={72} width={72} />
                </td>
                <td>
                  <h5 className="mt-0 mb-1 text-truncate">
                    <Link to="/apps/ecommerce/product-detail" className="text-body">
                      {item.name}
                    </Link>
                  </h5>
                  <p className="mb-0">
                ...
                <td>${item.price}</td>
                <td>
                  <input type="number" min="1" value={item.qty} className="form-control" placeholder="Qty" style={{
                  width: "90px"
                }} onChange={e => {
                  onQtyChange(e, item);
                }} />
                ...
                <td>${item.total}</td>
                <td>
                  <Link to="" className="action-icon text-danger" onClick={e => {
                  removeItem(e, item);
                }}>
                    {" "}
      ...
      <Row className="mt-4">
        <Col sm={6}>
          <Link to="/apps/ecommerce/products" className="btn text-muted d-none d-sm-inline-block btn-link fw-semibold">
