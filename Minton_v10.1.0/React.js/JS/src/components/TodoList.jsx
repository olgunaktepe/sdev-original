            ...
            {addTodos ? <>
                ...
                <Row>
                  ...
                  maxHeight: height
                      ...
                      return <li key={index} className="list-group-item border-0 ps-0">
                              {todo.done ? <div className="form-check mb-0">
                                  <input type="checkbox" className="form-check-input todo-done" id={String(todo.id)} checked={todo.done} onChange={() => toggleTodo(todo, index)} />
                                  <label className="form-check-label" htmlFor={String(todo.id)}>
                                    <s>{todo.text}</s>
                                  </label>
                                </div> : <div className="form-check mb-0">
                                  <input type="checkbox" className="form-check-input todo-done" id={String(todo.id)} checked={todo.done} onChange={() => toggleTodo(todo, index)} />
                                  <label className="form-check-label" htmlFor={String(todo.id)}>
                                    {todo.text}
                                  </label>
                ...
                <Row>
                  <Col>
                    <form name="todo-form" id="todo-form" onSubmit={handleSubmit(saveTodo)} className="mt-3">
                      <Row>
                        <Col>
                          <FormInput type="text" name="newTodo" className="form-control" placeholder="Add new todo" register={register} key="newTodo" errors={errors} control={control} />
                        </Col>
                        <Col>
