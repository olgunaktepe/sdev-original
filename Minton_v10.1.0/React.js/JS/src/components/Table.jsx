...
import { useAsyncDebounce, useExpanded, useGlobalFilter, usePagination, useRowSelect, useSortBy, useTable } from "react-table";
...
const GlobalFilter = ({
  ...
  return <div className={classNames(searchBoxClass)}>
      ...
      }} placeholder={`${count} records...`} className="form-control react-table-search ms-1" />
      </span>
        </div>;
};
...
const Table = props => {
  ...
  return <>
            ...
            <div className="table-responsive">
                <table {...dataTable.getTableProps()} className={classNames("table table-centered react-table", props["tableClass"])}>
                    ...
                    {(dataTable.headerGroups || []).map((headerGroup, idx) => <tr {...headerGroup.getHeaderGroupProps()} key={idx}>
                            {(headerGroup.headers || []).map((column, idx) => <th {...column.getHeaderProps(column.sort && column.getSortByToggleProps())} className={classNames({
            ...
            return <tr {...row.getRowProps()} key={index}>
                ...
                return <td {...cell.getCellProps()} key={index}>
                                            {cell.render("Cell")}
                                        </td>;
              })}
                            </tr>;
